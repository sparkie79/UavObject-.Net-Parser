...
namespace UavTalk
{
    public static class uavConsts
    {
        public  const byte SYNC_VAL = 0x3C;

        public static readonly byte[] crc_table = { 0x00, 0x07, 0x0e, 0x09, 0x1c,
        ...
        public const int TYPE_MASK = 0xF8;
        public const int TYPE_VER = 0x20;
        //! Packet contains an object
        public const int TYPE_OBJ = (TYPE_VER | 0x00);
        //! Packet is a request for an object
        public const int TYPE_OBJ_REQ = (TYPE_VER | 0x01);
        //! Packet is an object with a request for an ack
        public const int TYPE_OBJ_ACK = (TYPE_VER | 0x02);
        //! Packet is an ack for an object
        public const int TYPE_ACK = (TYPE_VER | 0x03);
        public const int TYPE_NACK = (TYPE_VER | 0x04);

        public const int MIN_HEADER_LENGTH = 8; // sync(1), type (1), size(2),
        // object ID(4)
        public const int MAX_HEADER_LENGTH = 10; // sync(1), type (1), size(2),
        // object ID (4), instance ID(2,
        // not used in single objects)

        public const int CHECKSUM_LENGTH = 1;

        public const int MAX_PAYLOAD_LENGTH = 256;

        public const int MAX_PACKET_LENGTH = (MAX_HEADER_LENGTH
                + MAX_PAYLOAD_LENGTH + CHECKSUM_LENGTH);

        public const int ALL_INSTANCES = 0xFFFF;
    }
}
