...
namespace UsbLibrary
{
    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
            ...
            if ( m_hHandle != InvalidHandleValue || m_hHandle == null)	// if the open worked...
			{
				...
				if (HidD_GetPreparsedData(m_hHandle, out lpData))	// get windows to read the device data into an internal buffer
				{
                    try
                    {
                        ...
                        m_nOutputReportLength = 2;//oCaps.OutputReportByteLength;	// ... and output report lengths
        ...
        {
            //int size = Marshal.SizeOf(oOutRep.Buffer);
            try
            {
                //m_oFile.Write(oOutRep.Buffer, 0, (int)oOutRep.BufferLength);
                oOutRep.Buffer[0] = 1;
                m_oFile.Write(oOutRep.Buffer, 0, 2);
            }
